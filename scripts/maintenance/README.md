...
- [Memory & swap (RAM sắp đầy)](#memory--swap-ram-sắp-đầy)
- [Security](#security)
...
## Memory & swap (RAM sắp đầy)

Khi server báo available memory rất thấp (vd. < 100MB), dễ dẫn tới OOM. Xử lý nhanh và tự động:

- **Chi tiết đầy đủ:** [MEMORY-RECOVERY.md](./MEMORY-RECOVERY.md)
- **Thêm swap ngay:** `sudo bash scripts/maintenance/setup-swap.sh`
- **Kiểm tra / tự xử lý khi critical:** `scripts/maintenance/memory-guard.sh` (chỉ log) hoặc `memory-guard.sh --auto` (restart ES khi cần)
- **Cron tự động:** chạy `memory-guard.sh --auto` mỗi 10 phút (xem hướng dẫn trong MEMORY-RECOVERY.md).
