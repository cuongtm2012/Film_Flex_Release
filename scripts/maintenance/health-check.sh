...
# Health Check Script for FilmFlex (canonical - single source of truth)
# Monitors system health and import effectiveness.
# scripts/deployment/health-check.sh is a wrapper that calls this script.
...
check_basic_system() {
    ...
    # Check memory (usage % and available MB - critical when available < 100MB)
    local memory_usage=$(free | awk 'NR==2{printf "%.0f", $3*100/$2}')
    local available_mb=$(free -m | awk 'NR==2{print $7}')
    if [ "$available_mb" -lt 100 ]; then
        log_health "ERROR" "CRITICAL: Available memory only ${available_mb}MB - risk OOM. Add swap or run scripts/maintenance/memory-guard.sh"
    elif [ "$memory_usage" -gt 90 ]; then
        log_health "WARNING" "High memory usage: ${memory_usage}%, available ${available_mb}MB"
    else
        log_health "SUCCESS" "Memory usage normal: ${memory_usage}%, available ${available_mb}MB"
    fi
