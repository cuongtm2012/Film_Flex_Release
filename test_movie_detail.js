// Test script for movie detail viewing
...
async function testMovieDetailViewing() {
  try {
    ...
    if (response.status === 200) {
      ...
      // Check the response structure and debug it
      console.log('Response structure:', Object.keys(data));
      
      // Adjust based on the actual response structure
      const movieItem = data.movie || data.item;
      
      if (movieItem) {
        console.log('Movie fields:', Object.keys(movieItem));
        
        // Check if the movie data has all the required fields
        const requiredFields = ['name', 'slug', 'description'];
        const missingFields = requiredFields.filter(field => !movieItem[field]);
      ...
      if (data.item.type === 'tv') {
        ...
        if (episodesResponse.status === 200 && episodesData.status === true) {
          ...
          if (episodesData.items.length > 0) {
            ...
            const requiredEpisodeFields = ['name', 'slug', 'link_embed'];
          ...
          console.log('❌ Test failed: Episodes endpoint returned an error');
        }
      } else {
